...
import shoppingCartItemSmall from "@/components/webshop-v3/shoppingCartItemSmall.vue";
...
const startPaymentByInstatag = (paymentType) => {
  if (window.window.WSC && window.window.WSC.modules.instatag) {
    let event = window.WSC.modules.instatag.createEvent("event91");
...
onMounted(() => {
  if (window.WSC && window.WSC.modules.instatag) {
    ...
    eventOrderReview.getEvar("eVarCartProductCategory").value =
      shoppingCartStore.products.map((product) => product.cat).join(",");
    eventOrderReview.getEvar("eVarCartProductStockStatus").value =
      shoppingCartStore.products
        .map((product) => product.stockStatus)
        .join(",");
